...
public final class SharedPipelineBridge implements PipelineBridge {
    ...
    public Channel channel(Player player) {
        ...
        try {
            var field = NetworkManager.class.getDeclaredField("i");
            field.setAccessible(true);
            return (Channel) field.get(network);
        } catch (ReflectiveOperationException error) {
            throw new IllegalStateException("Failed to read NetworkManager channel", error);
        }
    }
