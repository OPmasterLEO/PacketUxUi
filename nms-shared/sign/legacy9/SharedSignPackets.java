...
public final class SharedSignPackets implements SignPacketBridge {
    ...
    private boolean send(Player player, SignOpenRequest request, boolean placeBlock) {
        ...
        setPosition(sign, pos);
        String[] lines = request.legacyLines();
        for (int i = 0; i < 4; i++) {
            sign.lines[i] = new ChatComponentText(lines[i]);
        }
        if (placeBlock) {
            player.sendBlockChange(loc, material(request.materialName()), (byte) 0);
        }
    ...
    public SignUpdate readUpdate(Object packet) {
        ...
        BlockPosition pos = update.a();
        String[] raw = update.b();
        return new SignUpdate(pos.getX(), pos.getY(), pos.getZ(), raw, true);
    }

    private static void setPosition(TileEntitySign sign, BlockPosition pos) {
        try {
            sign.getClass().getMethod("setPosition", BlockPosition.class).invoke(sign, pos);
            return;
        } catch (ReflectiveOperationException ignored) {
        }
        try {
            sign.getClass().getMethod("a", BlockPosition.class).invoke(sign, pos);
        } catch (ReflectiveOperationException ignored) {
        }
    }
